...
.addField(`Alone | Prefix`, `**Cool MedyaBotun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Cool Medya| Nasıl Kullanılır?`, `**Cool Medyabotun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
...
.setDescription(`<:a_:821738957997211659>  <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Cool MedyaBot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Cool MedyaBot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
